...
-- 1. 회원가입
INSERT INTO MEMBERS(MID,MPW,MNAME,MGENDER,MBIRTH,MEMAIL)
VALUES ('TESTID', '1111', '테스트', 1, TO_DATE('2022-08-03','YYYY-MM-DD'), 'TESTID@icia.co.kr');
COMMIT;
...
-- 5. 상품 주문 요청[3개 주문]
INSERT INTO ORDERLIST (ODNUM, ODMID, ODDATE, ODPDCODE, ODQTY)
VALUES ((SELECT NVL(MAX(ODNUM), 0) + 1 FROM ORDERLIST), 'TESTID', SYSDATE, 'P0012', 3);
...
-- 6. 주문 내역 확인 :: [TESTID] 회원의 전체 주문 내역 조회
SELECT MEMBERS.MNAME AS 주문자, PRODUCT.PDNAME AS 상품명, PRODUCT.PDPRICE AS 가격, ORDERLIST.ODQTY AS 주문수,
(ORDERLIST.ODQTY * PRODUCT.PDPRICE) AS 결제금액, TO_CHAR(ORDERLIST.ODDATE,'YYYY-MM-DD') AS 주문일
FROM ORDERLIST
INNER JOIN MEMBERS ON MEMBERS.MID = ORDERLIST.ODMID
INNER JOIN PRODUCT ON PRODUCT.PDCODE = ORDERLIST.ODPDCODE
WHERE ORDERLIST.ODMID = 'TESTID' AND TO_CHAR(ORDERLIST.ODDATE,'YYYY-MM-DD') = '2022-09-16';

/* [주문취소] */
-- 1. PRODUCT 테이블의 취소된 주문의 상품의 수량을 증가
UPDATE PRODUCT
SET PDAMOUNT = PDAMOUNT + (SELECT ODQTY FROM ORDERLIST WHERE ODNUM = 3)
WHERE PDCODE = (SELECT ODPDCODE FROM ORDERLIST WHERE ODNUM = 3);
-- 2. ORDERLIST테이블의 취소한 주문 내역 삭제
...
SELECT *
FROM PRODUCT
WHERE PDNAME LIKE '%제로%';
/* 판매중인 상품 목록 : 상품 종류 */
SELECT *
FROM PRODUCT
WHERE PDTYPE LIKE '%음료%';
/* 재고가 남아있는 상품 목록 */
SELECT *
FROM PRODUCT
WHERE PDAMOUNT >= 1;
...
INSERT INTO SCORE (NO,STUNAME,KOREAN,ENGLISH,MATH,TOTAL,AVERAGE,GRADE)
VALUES(2,'이승원2',90,85,100,90+85+100,(90+85+100)/3,'B');

UPDATE SCORE SET KOREAN = 80 WHERE NO = 1;

DELETE FROM SCORE WHERE NO = 1;
